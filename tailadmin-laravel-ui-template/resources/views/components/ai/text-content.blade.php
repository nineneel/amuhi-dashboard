<div
    x-data="{
        ...
        async copyMessage(message) {
            try {
                ...
                this.copiedMessages.push(message.id);
                setTimeout(() => {
                    this.copiedMessages = this.copiedMessages.filter(id => id !== message.id);
                }, 2000);
            } catch (err) {
    ...
    class="custom-scrollbar relative z-20 max-h-[50vh] flex-1 space-y-7 overflow-y-auto pb-7"
>
    <template x-for="message in messages" :key="message.id">
        <div :class="message.isUser ? 'flex justify-end' : 'flex justify-start'">
            <div>
                <div
                    class="shadow-theme-xs max-w-[480px] rounded-xl px-4 py-3"
                    :class="message.isUser
                        ? 'bg-brand-100 dark:bg-brand-500/20 rounded-tr-xs'
                        : 'bg-gray-100 dark:bg-white/5 rounded-tl-xs'"
                >
                    <template x-for="(paragraph, index) in message.text" :key="index">
                        <p
                ...
                <div x-show="!message.isUser" class="mt-3">
                    <button
                    ...
                    >
                        <svg
                            x-show="!copiedMessages.includes(message.id)"
                        ...
                        <span x-text="copiedMessages.includes(message.id) ? 'Copied' : 'Copy'"></span>
                    </button>
                </div>
            </div>
        </div>
