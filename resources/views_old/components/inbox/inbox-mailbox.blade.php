...
>
    ...
    <div class="flex flex-col justify-between gap-3 border-b border-gray-200 p-4 dark:border-gray-800 sm:flex-row">
        <div class="flex w-full items-center gap-2">
            ...
            <div x-data="{ openDropDown: false, allChecked: false }" class="relative w-full sm:w-auto">
                <div class="flex w-full items-center justify-between gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-800">
                    <!-- LEFT: checkbox (no bubbling) -->
                    <label for="checkboxAll" class="flex cursor-pointer select-none items-center font-medium" @click.stop>
                        <input
                            type="checkbox"
                            id="checkboxAll"
                            ...
                            @click.stop
                        />
                        <span class="box flex h-4 w-4 items-center justify-center rounded-sm border border-gray-300 bg-transparent text-white dark:border-gray-700">
                    ...
                    <!-- RIGHT: dropdown toggle (separate button) -->
                    <button
                        @click="openDropDown = !openDropDown"
                        class="ml-auto text-gray-500 duration-300 ease-linear hover:text-gray-700 dark:text-gray-400 dark:hover:text-white"
                        :class="openDropDown ? 'text-gray-700 dark:text-white' : ''"
                        aria-expanded="false"
                        aria-haspopup="true"
                    >
                        <svg
                            class="stroke-current transition-transform duration-300"
                        ...
                        >
                            <path d="M3.83325 5.91699L7.99992 10.0837L12.1666 5.91699" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </button>
                </div>
            ...
            <div x-data="{ openDropDown: false }" class="relative w-full max-w-10">
                ...
                <div
                    ...
                    @click.outside="openDropDown = false"
                    x-transition
                    class="absolute right-0 top-full z-40 mt-1 w-40 space-y-1 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-lg transition-colors hover:bg-gray-100 dark:border-gray-800 dark:bg-gray-800 dark:hover:bg-gray-dark"
                    style="display: none;"
                >
                    <button class="flex w-full rounded-lg px-3 py-2 text-left text-theme-xs font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                        View More
        ...
        <div class="custom-scrollbar block h-full max-h-full overflow-auto">
            @foreach($mails as $index => $mail)
                ...
                >
                    <div class="flex w-3/5 items-center sm:w-1/5">
                        <label for="checkbox-{{ $index }}" class="cursor-pointer select-none">
                            <span class="relative">
                                ...
                                />
                                <span class="box mr-3 flex h-4 w-4 items-center justify-center rounded-sm border border-gray-300 bg-transparent text-white dark:border-gray-700">
                        ...
                        <div class="flex items-center">
                            <span class="pr-3 text-gray-400" @click="isStarred = !isStarred">
                                <svg
                                    class="fill-current"
                                    :class="isStarred ? 'hidden' : 'block'"
                                    width="20"
                                    height="20"
                                    viewBox="0 0 20 20"
                                    fill="none"
                                ...
                                <svg
                                    ...
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path d="M9.99991 3.125L12.2337 7.65114L17.2286 8.37694L13.6142 11.9L14.4675 16.8747L9.99991 14.526L5.53235 16.8747L6.38558 11.9L2.77124 8.37694L7.76613 7.65114L9.99991 3.125Z" fill="#FDB022"/>
                                </svg>
                            </span>

                            <span class="truncate text-sm text-gray-700 dark:text-gray-400">
                                {{ $mail['subject'] }}
                            </span>
                        </div>
                    </div>
                        ...
                        @if($mail['badge'])
                            <span
                                class="hidden rounded-full px-2 py-0.5 text-theme-xs font-medium sm:inline-block
                                    @if($mail['badge'] === 'Important')
                                        text-error-700 dark:text-error-500 bg-error-50 dark:bg-error-500/15
                                    @elseif($mail['badge'] === 'Social')
                                        text-success-700 dark:text-success-500 bg-success-50 dark:bg-success-500/15
                                    @elseif($mail['badge'] === 'Promotional')
                    ...
                    <div class="w-1/5">
                        <span class="block text-right text-theme-xs text-gray-400">
                            {{ $mail['time'] }}
                        </span>
                    </div>
                </div>
    ...
    {{-- Footer Pagination --}}
    <div class="sticky bottom-0 flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-[#171f2f]">
        <p class="text-sm text-gray-500 dark:text-gray-400">Showing 1 of 159</p>
        <div class="flex items-center justify-end gap-2">
            <button class="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400 dark:hover:bg-white/[0.03]">
