...
@section('content')
    ...
    @if ($errors->any())
        <x-ui.alert variant="error" title="Unable to update profile" :message="$errors->first()" class="mb-6" />
    @endif
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <x-form.input label="Email" type="email" name="email" id="email" :value="old('email', $user->email)" required />
                <x-form.input label="Phone" type="text" name="phone" id="phone" :value="old('phone', $profile?->phone)" required />
                <x-form.input label="Company Name" type="text" name="company_name" id="company_name" :value="old('company_name', $profile?->company_name)" />
                ...
                <div class="sm:col-span-2">
                    <label for="bio" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Bio / Description</label>
                ...
                <div class="sm:col-span-2"
                    ...
                    <label for="photo" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Profile Photo / Company Logo</label>
                    <div class="mt-3 flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                            ...
                            <span x-show="!previewUrl" class="text-xs font-medium text-gray-500 dark:text-gray-400">No photo</span>
                        </div>
                        <div class="w-full">
                            <x-form.input type="file" name="photo" id="photo" accept="image/png,image/jpeg,image/jpg"
                                x-on:change="updatePreview($event)" />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Accepted formats: JPG, JPEG, PNG. Max 2MB.</p>
                        </div>
                    </div>
                </div>
            </div>
