@props(['channels' => [], 'title' => 'Top Channels', 'reportLink' => '#', 'reportText' => 'Channels Report'])
...
@php
    $defaultChannels = [
        ...
        ['source' => 'Twitter', 'visitors' => '1.5K'],
    ];
    
    $channelsList = !empty($channels) ? $channels : $defaultChannels;
@endphp
    ...
    <div class="my-6">
        ...
        @foreach($channelsList as $channel)
            <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800">
                <span class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $channel['source'] }}</span>
                <span class="text-right text-gray-500 text-theme-sm dark:text-gray-400">{{ $channel['visitors'] }}</span>
            </div>
        @endforeach
    </div>

    <a href="{{ $reportLink }}" class="flex justify-center gap-2 rounded-lg border border-gray-300 bg-white p-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
        {{ $reportText }}
