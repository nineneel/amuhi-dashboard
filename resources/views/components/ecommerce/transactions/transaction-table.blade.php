                ...
                <tr class="border-b border-gray-200 dark:divide-gray-800 dark:border-gray-800">
                    ...
                    <th class="p-4 text-left text-xs font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                        <div class="flex cursor-pointer items-center gap-3" @click="sortBy('customer')">
                            ...
                            <span class="flex flex-col gap-0.5">
                                <svg :class="sort.key === 'customer' && sort.asc ? 'text-gray-800 dark:text-gray-400' :
                                    'text-gray-300'" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <th class="p-4 text-left text-xs font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                        <div class="flex cursor-pointer items-center gap-3" @click="sortBy('email')">
                            ...
                            <span class="flex flex-col gap-0.5">
                                <svg :class="sort.key === 'email' && sort.asc ? 'text-gray-800 dark:text-gray-400' :
                                    'text-gray-300'" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="currentColor" />
                                </svg>
                    ...
                    <th class="p-4 text-left text-xs font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                        <div class="flex cursor-pointer items-center gap-3" @click="sortBy('amount')">
                            ...
                            <span class="flex flex-col gap-0.5">
                                <svg :class="sort.key === 'amount' && sort.asc ? 'text-gray-800 dark:text-gray-400' :
                                    'text-gray-300'" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="currentColor" />
                                </svg>

                                <svg :class="sort.key === 'amount' && !sort.asc ? 'text-gray-800 dark:text-gray-400' :
                                    'text-gray-300'" width="8" height="5" viewBox="0 0 8 5" fill="none"
                        ...
                        <td class="p-4 whitespace-nowrap">
                            <div class="group flex items-center gap-3">
                                <label
                                    class="flex cursor-pointer items-center text-sm font-medium text-gray-700 select-none dark:text-gray-400">
                                    <span class="relative">
                                        <input type="checkbox" class="sr-only" :value="row.id" x-model="selected" @change="updateSelectAll()" />
                                        <span :class="selected.includes(row.id) ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                                            <span :class="selected.includes(row.id) ? '' : 'opacity-0'">
                                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </span>
                                        </span>
                                    </span>
                                </label>
                                <a href="/single-transaction" class="text-theme-xs font-medium text-gray-700 group-hover:underline dark:text-gray-400" x-text="row.orderId"></a>
                            </div>
                        </td>
                        <td class="p-4 whitespace-nowrap">
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-400" x-text="row.customer"></span>
                        </td>
                        <td class="p-4 whitespace-nowrap">
                            <p class="text-sm text-gray-500 dark:text-gray-400" x-text="row.email"></p>
                        </td>
                        <td class="p-4 whitespace-nowrap">
                            <p class="text-sm text-gray-700 dark:text-gray-400" x-text="row.amountDisplay"></p>
                        </td>
                        <td class="p-4 whitespace-nowrap">
                            <p class="text-sm text-gray-700 dark:text-gray-400">
                                <span x-text="row.dueDate"></span>
                            </p>
                        </td>
                        <td class="p-4 whitespace-nowrap">
                            <span
                            ...
                            <span
                                ...
                                x-show="row.status === 'Failed'">Failed</span>
                        </td>
                        <td class="p-4 whitespace-nowrap">
...
<script>
    function transactionTableFull() {
        return {
            ...
            get startEntry() {
                return this.rows.length === 0 ? 0 : (this.page - 1) * this.perPage + 1;
            },
            get endEntry() {
                let end = this.page * this.perPage;
                return end > this.rows.length ? this.rows.length : end;
            },
            goToPage(n) {
