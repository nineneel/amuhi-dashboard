@php
    $taskGroups = [
        ['name' => 'All', 'label' => 'All Tasks', 'count' => $allTasksCount ?? 23],
        ['name' => 'Todo', 'label' => 'To do', 'count' => $todoCount ?? 3],
        ['name' => 'InProgress', 'label' => 'In Progress', 'count' => $inProgressCount ?? 6],
        ['name' => 'Completed', 'label' => 'Completed', 'count' => $completedCount ?? 14],
    ];
@endphp
<div x-data="{saveTask(){
    console.log('Saving task...');
}}">
    <div class="flex flex-col items-center px-4 py-5 xl:px-6 xl:py-6" x-data="{
        ...
        <div class="flex flex-col w-full gap-5 sm:justify-between xl:flex-row xl:items-center">
            ...
            <div class="flex flex-wrap items-center gap-x-1 gap-y-2 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-900">
                @foreach ($taskGroups as $group)
                    <button
                        ...
                        :class="selectedTaskGroup === '{{ $group['name'] }}' ?
            ...
            <div class="flex flex-wrap items-center gap-3 xl:justify-end">
                ...
                <!-- Add New Task Button -->
                <button @click="$dispatch('open-task-modal')" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">
                    Add New Task
                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
    ...
    <!-- Task Modal -->
    <x-ui.modal x-data="{ open: false }" @open-task-modal.window="open = true" :isOpen="false" class="max-w-[700px]" x-cloak>
        <div
                ...
                <div class="custom-scrollbar h-[450px] overflow-y-auto px-2">
                    <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pl-4 pr-11 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                        </div>

                        <div>
                            <x-form.date-picker id="select-date" label="Select Date" placeholder="Select date" name="select-date" />
                        </div>
