...
@section('content')
    @php
        ...
        $upcomingCount = $events
            ->filter(fn($event) => $event->status?->value === \App\EventStatus::Upcoming->value)
            ->count();
        $ongoingCount = $events
            ->filter(fn($event) => $event->status?->value === \App\EventStatus::Ongoing->value)
            ->count();
        ...
        <div class="space-y-6 xl:col-span-8">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                {{-- <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('ui.events.event_list') }}
                    </h3>
                </div> --}}
                ...
                @else
                    <div class="space-y-4">
                        @foreach ($events as $event)
                            ...
                            @endphp
                            <div
                                class="task rounded-xl border border-gray-200 bg-white p-5 shadow-theme-sm transition-all hover:shadow-lg dark:border-gray-800 dark:bg-white/5">
                                <div class="flex items-start gap-4">
                                    @if ($eventImage)
                                        <div
                                            class="h-64 w-52 shrink-0 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-start justify-between gap-3">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <span
                                                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {{ $statusStyles[$status] ?? $statusStyles['upcoming'] }}">
                                                    {{ $statusLabel }}
                                                </span>
                                                @if ($isRegistered)
                                                    <span
        ...
        <div class="space-y-6 xl:col-span-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                <h3 class="mb-3 text-base font-semibold text-gray-800 dark:text-white/90">
                    {{ __('ui.events.my_registered_events') }}</h3>
                        ...
                        @foreach ($registeredEvents as $event)
                            ...
                            <a href="{{ route('events.show', $event) }}"
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $event->title }}
                                        </p>
