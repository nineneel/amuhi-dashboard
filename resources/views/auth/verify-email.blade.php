...
@section('content')
    <div class="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-12">
        ...
        <div class="relative z-10 w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-8 shadow-theme-lg dark:border-gray-800 dark:bg-gray-900 sm:p-10">
            <h1 class="mb-3 text-2xl font-semibold text-gray-900 dark:text-white">{{ __('ui.auth_extra.verify_email_title') }}</h1>
            <p class="mb-6 text-sm text-gray-600 dark:text-gray-400">
                {{ __('ui.auth_extra.verify_email_subtitle') }}
            </p>

            @if (session('status'))
                <x-ui.alert variant="success" :title="__('ui.auth_extra.email_sent')" :message="session('status')" class="mb-5" />
            @endif
            ...
            @if ($errors->any())
                <x-ui.alert variant="error" :title="__('ui.auth_extra.verification_issue')" :message="$errors->first()" class="mb-5" />
            @endif
            ...
            <div class="space-y-3">
                <form method="POST" action="{{ route('verification.resend') }}">
                    ...
                    <button type="submit"
                        ...
                        {{ __('ui.auth_extra.resend_verification_link') }}
                    </button>
                </form>
