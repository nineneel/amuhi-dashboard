...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">{{ __('ui.admin.event_details') }}</h1>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="lg:col-span-1">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    @endif
                    <h2 class="mb-5 text-base font-semibold text-gray-800 dark:text-white/90">{{ __('ui.admin.event_info') }}</h2>
                    <ul class="divide-y divide-gray-100 dark:divide-gray-800">
                        <li class="flex items-start gap-5 py-2.5">
                            <span class="w-1/3 text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.status') }}</span>
                            <span class="w-2/3">
                                @php
                        ...
                        <li class="flex items-start gap-5 py-2.5">
                            <span class="w-1/3 text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.location') }}</span>
                            <span class="w-2/3 text-sm text-gray-700 dark:text-gray-300">{{ $event->location ?? '-' }}</span>
                        </li>
                        <li class="flex items-start gap-5 py-2.5">
                            <span class="w-1/3 text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.starts') }}</span>
                            <span class="w-2/3 text-sm text-gray-700 dark:text-gray-300">{{ $event->starts_at?->format('d M Y, H:i') ?? '-' }}</span>
                        </li>
                        <li class="flex items-start gap-5 py-2.5">
                            <span class="w-1/3 text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.ends') }}</span>
                            <span class="w-2/3 text-sm text-gray-700 dark:text-gray-300">{{ $event->ends_at?->format('d M Y, H:i') ?? '-' }}</span>
                        </li>
                        <li class="flex items-start gap-5 py-2.5">
                            <span class="w-1/3 text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.registrations') }}</span>
                    ...
                    @if ($event->description)
                        <div class="mt-4 border-t border-gray-100 pt-4 dark:border-gray-800">
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.description') }}</p>
            ...
            <div class="lg:col-span-2">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <h2 class="mb-5 text-base font-semibold text-gray-800 dark:text-white/90">{{ __('ui.admin.registrations') }}</h2>
                    @if ($event->eventRegistrations->isEmpty())
                        <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('ui.admin.no_registrations_yet') }}</p>
                    @else
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-100 dark:border-gray-800">
                                        ...
                                        <th class="pb-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('ui.admin.name') }}</th>
                                        <th class="pb-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('ui.admin.email') }}</th>
                                        <th class="pb-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('ui.admin.registered') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
