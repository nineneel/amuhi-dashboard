...
@php
    ...
    $trendType = $trendValue === null ? 'neutral' : ($trendValue >= 0 ? 'success' : 'danger');
@endphp

<div {{ $attributes->class(['rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6']) }}>
    <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 text-brand-600 dark:bg-gray-800 dark:text-brand-400">
        @if ($icon)
            {!! $icon !!}
        @else
    ...
    <div class="mt-5 flex items-end justify-between">
        ...
        @if ($trend !== null)
            @include('admin.components.badge', [
                'type' => $trendType,
                'text' => ($trendValue !== null && $trendValue > 0 ? '+' : '') . $trend . '%',
            ])
        @endif
    </div>
</div>
