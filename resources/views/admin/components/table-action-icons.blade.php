...
@php
    $resolvedDeleteConfirm = __($deleteConfirm ?? 'Are you sure?');
    $resolvedDeleteLabel = __($deleteLabel ?? 'Delete');
    $resolvedViewLabel = __($viewLabel ?? 'View');
    $resolvedEditLabel = __($editLabel ?? 'Edit');
@endphp

<div class="flex items-center gap-1">
    @if ($viewUrl)
        <a href="{{ $viewUrl }}" aria-label="{{ $resolvedViewLabel }}"
    ...
    @if ($editUrl)
        <a href="{{ $editUrl }}" aria-label="{{ $resolvedEditLabel }}"
    ...
    @if ($deleteUrl)
        <form method="POST" action="{{ $deleteUrl }}" onsubmit="return confirm('{{ $resolvedDeleteConfirm }}')">
            @csrf
            @method('DELETE')
            <button type="submit" aria-label="{{ $resolvedDeleteLabel }}"
